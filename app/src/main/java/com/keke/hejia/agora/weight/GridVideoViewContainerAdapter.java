package com.keke.hejia.agora.weight;
...
//        移除修改后的 人
        ...
        while (it.hasNext()) {
            ...
            if (uids.get(status.mUid) == null) {
                LogUtils.loge("after_changed remove not exited members " + (status.mUid & 0xFFFFFFFFL) + " " + status.mView);
                it.remove();
            }
        }
...
//        进行重新 绘制
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        LogUtils.logd("onBindViewHolder " + position + " " + user + " " + myHolder + " " + myHolder.itemView);
